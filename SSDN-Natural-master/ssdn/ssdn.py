...
class NoiseNetwork(nn.Module):
    ...
    def __init__(
        ...
        def _max_pool_block(max_pool: nn.Module) -> nn.Module:
            if blindspot:
                return nn.Sequential(utils.Shift2d((1, 0)), max_pool)
            return max_pool
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.blindspot:
            rotated = [utils.rotate(x, rot) for rot in (0, 90, 180, 270)]
...
class ShiftConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        shift = utils.Shift2d(self.shift_size)
        self.pad = shift.pad
